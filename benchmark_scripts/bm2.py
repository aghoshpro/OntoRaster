...
NUM_OF_POINTS = 100 # 1,2 points are not polygon
# AREA_IN_km = 100 
UNIFORM_FACTOR = 0.4
XTICKS_BIN = 10
...
for AREA_IN_km in tqdm(np.arange(1, calculateArea(INPUT_BBOX_WKT_STRING), 1)):
    try:
        sample_polygon = genPolygonExp(INPUT_BBOX, NUM_OF_POINTS, AREA_IN_km, UNIFORM_FACTOR)
        time_taken = benchmark(sample_polygon)
        del sample_polygon
        gc.collect()
        benchmark_data.append({'AREA_IN_km': AREA_IN_km, 'Processing_Time': time_taken})
    except Exception as e:
        print(f"Error with AREA_IN_km {AREA_IN_km}: {e}")

df = pd.DataFrame(benchmark_data)
df.to_csv('./benchmark_scripts/bm2.csv', index=False)

### VISUALIZATION STARTS HERE ###

# df = pd.read_csv('./benchmark_scripts/bm2.csv')
...
bars = plt.bar(df['AREA_IN_km'], df['Processing_Time'], align='center', color=colors, width=1)
...
plt.title('Query Processing Time VS Varying AREA_IN_km of Fixed Area Uniform Polygon (Case 1)', fontsize=20, fontweight='bold')
plt.xlabel('AREA_IN_km', fontsize=15, fontweight='bold')
plt.ylabel('Processing Time (seconds)', fontsize=15, fontweight='bold')
plt.xticks(np.arange(df['AREA_IN_km'].min(), df['AREA_IN_km'].max() + 1, XTICKS_BIN), fontsize=12, rotation = 35)
plt.yticks(fontsize=15)
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.margins(x=0)
plt.savefig('./benchmark_scripts/bm2_bar_plot.png')

# plt.figure(figsize=(20, 10))
# bars = plt.bar(df['AREA_IN_km'], df['Processing_Time'], color=colors, align='center', width=1)
# # Label only min and max bars
# for bar in bars:
#     height = bar.get_height()
#     if height == min_height or height == max_height:
...
# plt.xticks(range(df['AREA_IN_km'].min(), df['AREA_IN_km'].max() + 1, 100))
# plt.xlabel('Varying AREA_IN_km of Polygon')
...
# # sns.barplot(data = df, x='AREA_IN_km', y='Processing_Time', palette="icefire")
# snscolors = sns.color_palette("Spectral_r", n_colors=len(df))
# ax = sns.barplot(data=df, x='AREA_IN_km', y='Processing_Time', palette=snscolors)
# plt.title('Processing Time by AREA_IN_km', fontsize=16, fontweight='bold', pad=20)
# plt.xlabel('Varying AREA_IN_km of Fixed Area Uniform Polygon', fontsize=12, labelpad=10)
# plt.ylabel('Processing Time (seconds)', fontsize=12, labelpad=10)
# plt.xticks(ticks=range(len(df)), labels=df['AREA_IN_km'], rotation=0)
# plt.grid(axis='y', linestyle='--', alpha=0.7)
...
# plt.plot(df['AREA_IN_km'], df['Processing_Time'], color='red', marker='o', linestyle='')
# plt.title('OntoRaster Performance Benchmark', fontsize=20, fontweight='bold')
# plt.xticks(range(df['AREA_IN_km'].min(), df['AREA_IN_km'].max() + 1, XTICKS_BIN))
# plt.xlabel('Varying AREA_IN_km of Polygon', fontsize=15, fontweight='bold')
# plt.ylabel('Processing Time (seconds)', fontsize=15, fontweight='bold')
# plt.grid(True)
# plt.margins(x=0)
...
# Plot the bar chart
plt.figure(figsize=(20, 10))
barsh = plt.bar(x, df['Processing_Time'], color=colors, align='center', width=1)
...
plt.tight_layout()
plt.margins(x=0)
plt.savefig('./benchmark_scripts/bm2_log_scale.png')
plt.show()
