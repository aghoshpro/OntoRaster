...
def benchmark(poly):
    ...
    query1 = f"""
        ...
        FILTER (CONTAINS(?rasterName, 'Munich_MODIS_SnowCover_500m')) 
    ...
    """
    # Munich_MODIS_Temperature_1km, NDVI, Snow
    # 2022-01-01T00:00:00+00:00, 2022-01-01T00:00:00+00:00, 2023-12-20T00:00:00+00:00
...
for point in tqdm(range(3, NUM_OF_POINTS, STEPS)):
    try:
        poly_bm500 = genPolygonExp(INPUT_BBOX, point, AREA_IN_km, UNIFORM_FACTOR)
...
bm500.to_csv('./benchmark_scripts/bm500_time.csv', index=False)
...
bm1.to_csv('./benchmark_scripts/bm1_time.csv', index=False)
...
bm11.to_csv('./benchmark_scripts/bm11_time.csv', index=False)
...
bm111.to_csv('./benchmark_scripts/bm111_time.csv', index=False)
...
bm1111.to_csv('./benchmark_scripts/bm1111_time.csv', index=False)
...
bm11111.to_csv('./benchmark_scripts/bm11111_time.csv', index=False)

# bm500 = pd.read_csv('./benchmark_scripts/bm500_100.csv')
# bm1 = pd.read_csv('./benchmark_scripts/bm1_100.csv')
# bm11 = pd.read_csv('./benchmark_scripts/bm11_100.csv')
# bm111 = pd.read_csv('./benchmark_scripts/bm111_100.csv')
# bm1111 = pd.read_csv('./benchmark_scripts/bm1111_100.csv')
# bm11111 = pd.read_csv('./benchmark_scripts/bm11111_100.csv')
...
dataframes = [bm500, bm1, bm11, bm111, bm1111, bm11111]
titles = ['500 km²', '250 km²', '100 km²', '50 km²', '25 km²', '5 km²']
...
for i, (df, title) in enumerate(zip(dataframes, titles)):
    if i < len(axes):  # Make sure we don't exceed the number of available subplots
        ...
        axes[i].set_yticks(np.arange(df['Processing_Time'].min(), df['Processing_Time'].max(), 0.1),  fontsize=15, fontweight='bold')
...
# fig.suptitle('POLYGON ( increasing points | fixed area | 0.1 uniformity)', fontsize=20, fontweight='bold')
...
for i, (df, title) in enumerate(zip(dataframes, titles)):
    if i < len(axes):  # Make sure we don't exceed the number of available subplots
        ...
        axes[i].set_title(title, fontsize=15, fontweight='bold')
        axes[i].set_xticks(range(df['Points'].min(), df['Points'].max() + 1, XTICKS_BIN), fontsize=15, fontweight='bold', rotation = 35)
        axes[i].set_yticks(np.arange(np.min(trend_y), np. max(trend_y), 0.1),  fontsize=15, fontweight='bold')
        axes[i].set_xlabel('Points', fontsize=15, fontweight='bold')
        axes[i].set_ylabel('Time (seconds)', fontsize=15, fontweight='bold')
        axes[i].grid(True, axis='y', linestyle='--', alpha=0.7)
        axes[i].margins(x=0)
        # axes[i].set_ylim(y_min_log, y_max_log)
