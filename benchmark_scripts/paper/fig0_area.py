...
def main(iteration=0):
    ...
    # Generate additional polygons with increasing points (fixed area)
    # for num_points in pts: 
    #     poly = genPolygon_AreaFIXED(INPUT_BBOX, num_points, PERCENTAGE_OF_AREA)
    #     polygons.append(poly)
    #     titles.append(f"#Points = {num_points}")

    # Generate additional polygons with increasing area (fixed num of points)
    for area_percentage in [0.024, 0.044,  0.1, 0.2095, 0.398, 0.65, 0.90, 1.3, 1.6, 1.99]: 
        poly = genPolygon_AreaFIXED(INPUT_BBOX, 25, area_percentage)
        polygons.append(poly)
        # titles.append(f"{int(area_percentage*100)}% --- {int(calculateArea(poly))} km2")
        titles.append(f"{int(calculateArea(poly))} km²")
